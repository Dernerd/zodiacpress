...
function zp_get_interps_section_title( $tab, $section ) {
	...
	switch ( $tab ) {
		case 'natal_planets_in_signs':
			$title = sprintf( __( 'Interpretationen für Geburt %1$s in den Zeichen', 'zodiacpress' ), $planet );
			break;
		case 'natal_planets_in_houses':
			$title = sprintf( __( 'Interpretationen für Geburt %1$s in den Häusern', 'zodiacpress' ), $planet );
			break;
		case 'natal_aspects':
			$title = sprintf( __( 'Interpretationen für Aspekte von Geburt %1$s', 'zodiacpress' ), $planet );
			break;
		default:
			$title = __( 'Geburts Interpretationen', 'zodiacpress' );
			break;
	}
	return $title;	
}
	...
	if ( ! empty( $options['enable_planet_houses'] ) ) {
		...
		foreach ( $options['enable_planet_houses'] as $planet ) {
			...
			for ( $i = 1; $i < 13; $i++ ) {
				...
				$name 	= sprintf( __( '%1$s im Haus von %2$s', 'zodiacpress' ),
							$planet['label'],
							$order[ $i ]
							);
...
function zp_interps_sanitize( $input = array() ) {
	...
	add_settings_error( 'zp-interps-notices', '', __( 'Interpretationen aktualisiert.', 'zodiacpress' ), 'updated' );

	return $output;
}
function zp_get_interps_tabs() {
	$tabs = array(
		'natal_planets_in_signs' 	=> __( 'In Zeichen', 'zodiacpress' ),
		'natal_planets_in_houses' 	=> __( 'In Häusern', 'zodiacpress' ),
		'natal_aspects'				=> __( 'Aspekte', 'zodiacpress' )
	);
	return $tabs;
}
